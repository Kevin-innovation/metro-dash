...
describe("한 학교가 한 줄로 모이는지", () => {
  ...
  it("DIS 는 초 · 중 · 고 어디서 검색해도 같은 표시 이름으로 정규화된다", () => {
...
describe("실제 학교 목록에서 나온 까다로운 이름들", () => {
  ...
  it("긴 정식 명칭은 사람들이 실제로 부르는 이름으로 줄인다", () => {
    // 「서울대학교사범대학부설」은 열일곱 자고 아무도 그렇게 말하지 않는다.
    // 지역의 국립대 부설이면 그냥 사대부이고, 그래서 보드에는 서울사대부초로
    // 오른다 — 그 학교가 실제로 불리는 이름이다.
    expect(ok("서울", "초", "서울대학교사범대학부설초등학교").label).toBe("서울사대부초");
    expect(ok("대구", "중", "경북대학교사범대학부설중학교").label).toBe("대구사대부중");
    expect(ok("부산", "고", "부산대학교사범대학부설고등학교").label).toBe("부산사대부고");

    // 그 지역의 국립대가 아니면 이름이 남는다. 서울에만 사범대 부설이 여럿이라
    // 이름을 떼면 전부 같은 학교가 된다.
    expect(ok("서울", "초", "상명대학교사범대학부속초등학교").school.name).toBe("상명대사대부");
  });
...
describe("예전 규칙으로 저장된 학교 되돌리기", () => {
  ...
  it("이름 안에 '학교'가 들어 있을 뿐인 줄은 학교급을 붙인다", () => {
    ...
    // 예전 규칙으로 저장된 정식 명칭은 되돌리면서 지금의 약칭까지 입는다.
    expect(
      canonicalSchool({
        region: "서울",
        level: "초",
