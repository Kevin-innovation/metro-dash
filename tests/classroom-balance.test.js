...
describe("20만 · 30만 · 40만 · 50만", () => {
  ...
  it("까마귀는 15만 — 코인 줄을 의심하기 시작할 때다", () => {
    // 70만이던 시절 초보는 3분 안에 까마귀를 한 번도 못 봤고, 50만으로 옮긴
    // 뒤에도 이미 이기고 있는 사람에게만 물어보는 질문이었다.
    //
    // 알은 새가 아니다. 옆에 빈 레인이 있는, 함정이 든 코인 줄이다. 그러니
    // 문턱이 정하는 건 「언제부터 안 보이기 시작하나」가 아니라 「언제부터
    // 이 코인 줄 정말 먹을 거냐고 묻나」이고, 그건 코인 줄을 아직 믿고 먹는
    // 시점이어야 의미가 있다. 보통 플레이로 45초 언저리다.
    expect(HAZARD_FROM_SCORE).toBe(150_000);
  });
