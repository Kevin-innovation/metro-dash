...
/**
 * What the speed pickups do to the curve.
 *
 * Takes the whole timer state rather than one boolean. There was briefly a
 * brake on the other end of this — it was not fun, and a pickup that takes
 * score away to buy time is a thing a player resents rather than chases — but
 * the shape it left behind is the right one: speed is a property of the run,
 * read off everything the player is carrying, not a flag Game has to remember
 * to pass.
 *
 * @param {object} timers live power-up state
 */
export function runSpeedFactor(timers) {
  return isActive(timers, "focus") ? SPRINT_SPEED : 1;
}
