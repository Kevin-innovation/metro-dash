...
export const CHANGELOG = [
  {
    ...
    title: "잘할수록 어려워집니다",
    notes: [
      "지금까지는 7분을 넘기면 게임이 더 이상 어려워지지 않았어요. 속도도 간격도 장애물 종류도 전부 상한에 닿아서, 그 뒤로는 같은 1분이 계속 반복됐습니다. 오래 버티기만 하면 점수가 끝없이 올라갔고, 순위표가 실력이 아니라 집중력이 버틴 시간을 재고 있었어요.",
      "이제 난이도가 시계가 아니라 점수를 따라갑니다. 5만 점부터 2만 5천 점마다 한 단계씩 올라가고, 올라갈 때마다 까다로운 배치가 더 자주 나옵니다. 잘하는 사람은 빨리 만나고, 아직 어려운 사람은 만나지 않아요.",
      "단계마다 새 배치가 열립니다. 한 레인만 열린 줄을 연달아 두 번 지나가야 하는 것, 게이트를 슬라이딩으로 지나자마자 레인을 바꿔야 하는 것, 점프·슬라이딩·레인 변경을 연속으로 요구하는 것까지 다섯 종류가 추가됐어요.",
      "단계가 오르면 화면에 알려 줍니다. 왜 갑자기 어려워졌는지 모른 채로 죽는 일이 없도록요.",
      "단계가 올라갈수록 1점의 값도 조금씩 내려갑니다. 배치를 어렵게 만드는 것만으로는 부족했어요 — 간격은 이미 물리적 한계까지 좁혀져 있어서, 더 좁히면 넘을 수 없는 자리가 생깁니다. 그래서 순위표 위쪽이 아래쪽의 40배까지 벌어져 있었습니다. 이제 그 차이가 14배 정도가 되고, 비슷한 실력끼리 같은 점수대에서 겨루게 됩니다.",
      "값이 내려가도 0이 되지는 않습니다. 끝까지 간 판은 계속 보상을 받아요. 5만 점까지는 아무 영향도 없어서, 대부분의 판은 지금과 똑같습니다.",
      "대신 처음 90초는 더 여유로워졌습니다. 순위표 아래쪽은 못하는 사람이 아니라 게임이 뭘 요구하는지 배우기도 전에 죽은 사람들이었어요. 시작 구간의 배치 간격을 넓혔습니다.",
      "새로 추가된 배치도 전부 피할 수 있습니다. 40판 700초를 돌려 장애물 39,818쌍을 검사했고, 피할 수 없는 자리는 하나도 없었어요.",
      "한 계정이 1분에 올릴 수 있는 기록 수에 상한을 뒀습니다. 연속으로 빨리 죽어도 걸리지 않는 값이에요.",
    ],
  },
  {
    version: "2.50",
